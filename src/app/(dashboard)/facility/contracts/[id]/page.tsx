...
const STATUS_LABEL: Record<string, string> = {
  active: 'Active',
  completed: 'Completed',
  pending_contractor: 'Pending Contractor Approval',
  pending_facility: 'Pending Your Approval',
  draft: 'Draft',
  cancelled: 'Cancelled',
...
export default function FacilityContractDetailPage() {
  ...
  const handleApproveTimesheet = async (timesheetId: string) => {
    if (isDemoMode()) {
      toast.success('Timesheet approved. (demo mode)')
      setTimesheets((prev) =>
        prev.map((t) => (t.id === timesheetId ? { ...t, status: 'approved' } : t)) as TimesheetItem[]
      )
      return
    }
    const supabase = createClient()
  ...
  const handleDisputeTimesheet = async (timesheetId: string) => {
    if (isDemoMode()) {
      toast.success('Timesheet disputed. (demo mode)')
      setTimesheets((prev) =>
        prev.map((t) => (t.id === timesheetId ? { ...t, status: 'disputed' } : t)) as TimesheetItem[]
      )
      return
    }
    const supabase = createClient()
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-3">
        <Button
          variant="ghost"
          size="icon-sm"
          render={<Link href="/facility/contracts" />}
        >
      ...
      {contract.status === 'pending_facility' && (
